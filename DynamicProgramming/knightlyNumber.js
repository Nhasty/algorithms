...
const knightlyNumber = (n, m, kr, kc, pr, pc, memo = {}) => {
  ...
  // define position
    //debugger
  const pos = kr + ',' + kc + ',' + m;
  // if position in memo return memo
  if (pos in memo) {
    return memo[pos];
  }
  // if moves = 0
  if (m === 0) {
    // if position equals target return 1
    if (kr === pr && kc === pc) {
      return 1;
    // else return 0
    } else {
  ...
  memo[pos] = 0;
  // get possible moves;
  const moves = validMoves(n, kr, kc);
  // iterate over possible moves
  for (let move of moves) {
    // increment memo[pos] with call to self at potential move, with m -1
    const [r, c] = move;
    memo[pos] += knightlyNumber(n, m - 1, r, c, pr, pc, memo);
  }
  return memo[pos];
  // return memo[pos];
};

const validMoves = (n, kr, kc) => {
  const deltas = [
    [kr + 1, kc + 2],
    [kr - 1, kc + 2],
    [kr + 2, kc + 1],
